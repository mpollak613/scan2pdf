...
static std::filesystem::path get_log_path()
{
    ...
    {
        log_path =  std::filesystem::path(xdg_data_home) / "scan2pdf/";
    }
    else
    {
        ...
        {
            log_path = std::filesystem::path(home_path) / ".local/share/scan2pdf/";
        }
    }
    #endif
...
void initialize_all(const char *program)
{
    ...
    MagickCore::SetLogName(std::string(get_log_path() / "magick.log").c_str());
    ...
    api->SetVariable("debug_file", std::string(get_log_path() / "tess.log").c_str());
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        {
            throw std::runtime_error("Could not open a scanner: " + std::string(sane_strstatus(sane_status)));
        }
        ...
        for (size_t i = 0; sane_start(sane_scanner_handle) != SANE_STATUS_NO_DOCS; ++i)
        {
            ...
            if (keep_image)
            {
                ...
                Pix *pimage = pixReadMemTiff((const unsigned char *)bimage.data(), bimage.length(), 0);
                api->SetImage(pimage);
